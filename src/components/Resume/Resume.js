...
const Resume = forwardRef((props, ref) => {
  ...
  const sectionDiv = {
    ...
    [sections.extracurricular]: (
      <div
        key={"extracurricular"}
        draggable
        onDragOver={() => seTarget(info.extracurricular?.id)}
        onDragEnd={() => setSource(info.extracurricular?.id)}
        className={`${styles.section} ${
          info.extracurricular?.sectionTitle ? "" : styles.hidden
        }`}
      >
        <div className={styles.sectionTitle}>{info.extracurricular?.sectionTitle}</div>
        <div className={styles.content}>
          <p className={styles.overview}>{info.extracurricular?.detail}</p>
        </div>
      </div>
    ),
    [sections.other]: (
  ...
  useEffect(() => {
    setColumns([
      [sections.project, sections.education, sections.extracurricular],
